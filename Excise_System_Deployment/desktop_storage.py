...
SPIRIT_TRANSACTION_EXCEL_FILE = EXCISE_FOLDER / "Spirit_Transaction_Data.xlsx"
...
# --- SPIRIT TRANSACTION STORAGE ---
def ensure_spirit_transaction_excel_exists():
    if not SPIRIT_TRANSACTION_EXCEL_FILE.exists():
        try:
            from spirit_transaction_schema import SPIRIT_TRANSACTION_COLUMNS
            df = pd.DataFrame(columns=SPIRIT_TRANSACTION_COLUMNS)
            df.to_excel(SPIRIT_TRANSACTION_EXCEL_FILE, index=False, sheet_name="Spirit Transaction")
        except ImportError:
            df = pd.DataFrame()
            df.to_excel(SPIRIT_TRANSACTION_EXCEL_FILE, index=False, sheet_name="Spirit Transaction")
    return SPIRIT_TRANSACTION_EXCEL_FILE


def get_spirit_transaction_data_from_excel():
    ensure_spirit_transaction_excel_exists()
    try:
        return pd.read_excel(
            SPIRIT_TRANSACTION_EXCEL_FILE, sheet_name="Spirit Transaction"
        ).dropna(how="all")
    except Exception:
        return pd.DataFrame()


def save_spirit_transaction_to_excel(df):
    try:
        df.to_excel(SPIRIT_TRANSACTION_EXCEL_FILE, index=False, sheet_name="Spirit Transaction")
        return True, "âœ… Data saved to Spirit Transaction Excel"
    except Exception as e:
        return False, str(e)


def add_spirit_transaction_record_to_excel(data_dict):
    df = get_spirit_transaction_data_from_excel()
    txn_date = str(data_dict.get("txn_date") or "")
    if txn_date and "txn_date" in df.columns:
        df = df[df["txn_date"].astype(str) != txn_date]
    new_row = pd.DataFrame([data_dict])
    df = pd.concat([df, new_row], ignore_index=True)
    success, msg = save_spirit_transaction_to_excel(df)
    return success, msg, txn_date
...
def init_all_excel_files():
    """Create all blank Excel files with headers if they don't exist"""
    print("📁 Initializing Desktop Excel Storage...")
    ensure_folder_exists()
    
    # Trigger all exists checks
    ensure_reg76_excel_exists()
    ensure_reg74_excel_exists()
    ensure_rega_excel_exists()
    ensure_reg78_excel_exists()
    ...
    print("✅ All Excel files initialized on Desktop.")
...
print(f"📊 Spirit Transaction Excel File: {SPIRIT_TRANSACTION_EXCEL_FILE}")
